import {gT} from "tia"
const { t, l, s, sOrig } = gT;
const eC = gT.eC;


async function test() {
  t.setTitle("Test for client exceptions");
  await s.driver.init(false, true)
  await s.driver.sleep(10000, true)
  await s.driver.sleep(10000, true)

  await eC.textfield.a.('menu > textfield')
  await eC.textfield.a.sendEnter()

  await gT.eC.component.a.clickInput('menu > menucheckitem#filters')
  await gT.eC.textfield.a.('menu > textfield', 'Next Generation')
  await gT.eC.component.a.click

  await gT.eC.gridcolumn.a.clickTrigger('#organization &grid > headercontainer > multiline_treecolumn[text="Наименование"]')
  await gT.eC.gridcolumn.a.('#organization &grid > headercontainer > multiline_treecolumn[text="Наименование"]')
  await gT.eC.textfield.a.('menu > textfield', 'Next Generation')

  await eC.combobox.a.setByMouse('#organization &form > usertagfield[name=admin_ids]', 'Gill, Sigourney (a179606@rvlab.net)')
  await eC.combobox.a.sendKeys('#organization &form > usertagfield[name=admin_ids]', 'ECS')
  await gT.eC.gridcolumn.a.click('#organization &grid > headercontainer > multiline_treecolumn[text="Наименование"]')
  await gT.eC.component.a.click('menu > menucheckitem#filters')
  await gT.eC.textfield.a.sendKeys('menu > textfield', 'Next Generation')
  await gT.e.wait.ajaxRequestsFinish(1000)  
  await s.uA.sendKeysById('')
  await s.browser.loadPage("https://test5-demo.rvision.pro/login", true)
  await s.wait.waitForElementEnabledAndVisibleById("username", 10000);
  await s.browser.executeScript(
    'var usr = document.querySelector("#username");var psw = document.querySelector("#password"); usr.value = "admin";psw.value = "admin"'
  );
  await gT.eC.textfield.a.sendKeys()
  l.println(`Result of script execution: ${res}`);
  await s.driver.sleep(5000, true);
  await s.uA.sendEnterById('submit')
  await gT.eC.boundlist.a.click('#organization &form > combobox[name=type_id] boundlist')

  await gT.eC.tab.a.click('#r-main > tabbar > tab[text=l"protective_system"]')
  await eC.combobox.a.click()
  await gT.s.browser.initTiaBrHelpers();
  await gT.e.initTiaExtJsBrHelpers();
  await  gT.eC.button.a.click
  ('#assets &tabsContainer > button[text="Организация"]')
  await gT.eC.textfield.a.sendKeys
('assets-organization-form > textarea[name=name]')
await gT.eC.form.a.click()
('#organization &form > textarea[name=name]', 'Next Generation')
await gT.eC.combobox.a.setByMouse('#organization &form > combobox[name=type_id]', )
await gT.eC.treeview.l.content('#organization &grid > treeview')
await eC.combobox.a.setByKbd('#organization &form > usertagfield[name=admin_ids]', "Hunter, Carl (a286176@rvlab.net)")
await gT.eC.boundlist.a.sendKeys('#organization &form > usertagfield[name=admin_ids] boundlist')
await gT.eC.boundlist.a.('assets-organization-form > usertagfield[name=admin_ids] boundlist')
// l.println('No exceptions and console logs:');
  // await s.browser.printCaughtExceptions(true);
  // await s.browser.printSelBrowserLogs();

  // await issueClientException();

  // l.println('One exception and one console log:');
  // await s.browser.printCaughtExceptions(true);
  // await s.browser.printSelBrowserLogs();

  // l.println('No exceptions and console logs:');
  // await s.browser.printCaughtExceptions(true);
  // await s.browser.printSelBrowserLogs();

  // await s.browser.close();
  await s.driver.quit(true);
}

module.exports = test();
